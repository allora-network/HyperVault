...
| 4 | Core spot USDC back to EVM idle | YES (proven live 2026-06-15/16) | Core USDC's declared EVM contract (0x6b9e...0a24) is Circle's CoreDepositWallet, the official USDC bridge live since 2025-12-08. A Core-side send of USDC to the system address 0x2000...0000 triggers the wallet's system-guarded `transfer(to, amount)`, paying native USDC (0xb883...630f) from its reserve to the **caller** (the vault). That Core-side send is what `pullFromCore` emits — **but as CoreWriter `send_asset` (action 13), NOT `spot_send` (action 6)**, which unified accounts silently drop (the spike caught this). The escape crank inherits two caveats: a ~0.00134 USDC withdrawal fee (so never request the exact full Core balance — it is dropped), and the same send_asset encoding. Options below. |
