...
| **G** | **RESOLVED in v1.5 (G2, 2026-06-12) — the "unusable bridge" was the wrong mechanism, not a missing one.** The original facts stand: `tokenInfo(0).evmContract = 0x6B9E…0A24 ≠ asset()`, and the system address `0x2000…0000` is blacklisted on the configured USDC, so a LEGACY push reverts (`test_G_legacyPushRevertsOnBlacklistedBridge`). But `0x6B9E…0A24` turned out to be **Circle's CoreDepositWallet — the official USDC EVM<->Core bridge (live 2025-12-08)**, deliberately not an ERC-20; the blacklist forces the wallet path. v1.5 reworks `pushToCore` to `approve+deposit` on the wallet (validated immutable, `CoreLinkVerified` attestation) and `pullFromCore` is unchanged (the wallet's system-guarded `transfer` pays the EVM side). See `SECURITY.md` §"v1.5 G2", `FORK_PROOFS.md` §"v1.5 G2", and Scenario C in the runbook. | `:351,493,516` | 🟢 RESOLVED | ✅ fork; ⏳ live Scenario C |
...
**v1.5 G2 addendum (2026-06-12).** Must-fix (1) is **closed, better than planned**: `0x6B9E…0A24` is Circle's CoreDepositWallet — the official USDC bridge (live since 2025-12-08) — so repatriation did not need a treasury redesign at all. `pushToCore` now deposits through the wallet, `pullFromCore` already emitted the correct Core-side action, and the trustless deposit→Core→trade→back→withdraw loop is restored with the shipped Circle USDC as the only deposit asset (issuer-trust caveat: the wallet is Circle-operated, upgradeable, pausable — both directions pause together; Path B stays documented as the contingency). Proofs: fork suite 54/0/4 incl. real-wallet-bytecode push custody; the funded Scenario C round-trip is the remaining gate before merge. Must-fix (4)'s permissionless leg is scoped in `ESCAPE_HATCH_SCOPE.md` and is unblocked by this route.
