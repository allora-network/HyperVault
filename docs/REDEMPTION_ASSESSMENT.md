...
| **G** | **RESOLVED in v1.5 (G2, 2026-06-12) — the "unusable bridge" was the wrong mechanism, not a missing one.** The original facts stand: `tokenInfo(0).evmContract = 0x6B9E…0A24 ≠ asset()`, and the system address `0x2000…0000` is blacklisted on the configured USDC, so a LEGACY push reverts (`test_G_legacyPushRevertsOnBlacklistedBridge`). But `0x6B9E…0A24` turned out to be **Circle's CoreDepositWallet — the official USDC EVM<->Core bridge (live 2025-12-08)**, deliberately not an ERC-20; the blacklist forces the wallet path. v1.5 reworks `pushToCore` to `approve+deposit` on the wallet (validated immutable, `CoreLinkVerified` attestation) and **reworks `pullFromCore` to CoreWriter `send_asset` (action 13)** — the legacy `spot_send` (action 6) is silently dropped by unified accounts (the live spike caught this); the wallet's system-guarded `transfer` then pays the caller (the vault). See `SECURITY.md` §"v1.5 G2", `FORK_PROOFS.md` §"v1.5 G2 — live spike", and Scenario C in the runbook. | `:351,493,516` | 🟢 RESOLVED | ✅ fork + live (round trip proven 2026-06-15/16) |
...
>   `coreSpotUsdc()` is the wallet-bridged NAV term. **Finding G RESOLVED + round trip
>   PROVEN LIVE (2026-06-15/16, G2):** `0x6B9E…0A24` is Circle's CoreDepositWallet, push
>   via `deposit`, pull via `send_asset` (action 13) — see `FORK_PROOFS.md` §"v1.5 G2 — live spike".
...
- **TODO-1 (Finding G) — ✅ CLOSED in v1.5 (G2) and PROVEN LIVE 2026-06-15/16, superseding the analysis below.** The 2026-06-03 conclusions were right about the observations and wrong about the interpretation: `0x6B9E…0A24` reverts ERC-20 reads because it is **Circle's CoreDepositWallet (the official bridge), not a token** — deposits go through `approve+deposit(amount, dex)` on it, and the Core-side send to `0x2000…0000` pays out through its system-guarded `transfer`. v1.5 reworks `pushToCore` accordingly (validated `coreDepositWallet` immutable). **The pull was ALSO reworked** — `pullFromCore`/`operatorRecoverSpot`/`emergencyRepatriate` now emit CoreWriter **`send_asset` (action 13)**, not the legacy `spot_send` (action 6), which unified HyperCore accounts silently drop (the live spike caught this: the EVM tx succeeds but Core never debits). Two operational nuances: a ~0.00134 USDC withdrawal fee means you must never pull the exact full Core balance (it is dropped — pull under it), and a vault's first push costs 1.0 USDC account-activation gas. The vault was also too big to deploy (26411 B vs HyperEVM's enforced 24576 limit) — the trade gate + emergency close moved into the external delegatecall library `VaultTradeLib` (vault now 24237 B; behaviour identical, fork 54/0/4, unit 10/10). Path B (`operatorRecoverSpot` → treasury → re-deposit) demotes to a **contingency** for a Circle-paused wallet. The funded round trip ran and closed in both directions — **no remaining gate** (tx hashes in `FORK_PROOFS.md`). <details>Original (superseded) text: the live spike settled both branches: the canonical bridge is dead for the shipped asset (`pushToCore` reverts `Blacklistable`), and the asset-swap escape is closed — `0x6B9E…0A24` reverts standard reads, so it cannot be an ERC-4626 asset. The only viable realisation path was thought to be Path B, exercised live.</details>
...
All nine findings are now **proven on real HyperEVM-mainnet bytecode** (`docs/FORK_PROOFS.md`) — with **F and Q4 closed on the live funded spike of 2026-06-03** (the two NAV>idle cases a fork cannot reach). **Finding G — once thought the headline blocker — is RESOLVED in v1.5 G2 (proven live 2026-06-15/16):** the configured asset's `tokenInfo(0).evmContract = 0x6B9E…0A24` is **Circle's CoreDepositWallet** (the official USDC bridge), not a broken/competing token. Push deposits through the wallet (`approve + deposit`); pull is a CoreWriter **`send_asset` (action 13)** to the system address — NOT the legacy `spot_send` (action 6), which unified accounts silently drop — and the wallet pays native USDC back to the vault. The full deposit→Core→trade→back→withdraw loop is proven (tx hashes in `FORK_PROOFS.md` §"v1.5 G2 — live spike"; the spike also fixed an EIP-170 size blocker via the `VaultTradeLib` delegatecall split). So the redemption queue **can** realise Core value back into `asset()` trustlessly; the real residual is the **queue hardening** below (keeper, on-chain deadline, escape hatch), not the bridge. Caveats now baked in: a ~0.00134 USDC withdrawal fee means the keeper never requests the exact full Core balance (dropped); a vault's first push costs 1.0 USDC one-time activation gas.
...
**v1.5 G2 addendum (2026-06-12; round trip PROVEN LIVE 2026-06-15/16).** Must-fix (1) is **closed, better than planned**: `0x6B9E…0A24` is Circle's CoreDepositWallet — the official USDC bridge (live since 2025-12-08) — so repatriation did not need a treasury redesign at all. `pushToCore` deposits through the wallet; **`pullFromCore` was reworked to CoreWriter `send_asset` (action 13)** — the original `spot_send` (action 6) is silently dropped by unified HyperCore accounts, which the funded spike caught and fixed (along with two nuances: a ~0.00134 USDC withdrawal fee, so never request the exact full Core balance, and a one-time 1.0 USDC first-push activation gas). The vault was also shrunk under the EIP-170 24576-byte limit by extracting the trade gate + emergency close into `VaultTradeLib` (delegatecall). The full trustless deposit→Core→trade→back→withdraw loop is now **proven live in both directions** with the shipped Circle USDC as the only deposit asset (issuer-trust caveat: the wallet is Circle-operated, upgradeable, pausable — both directions pause together; Path B stays documented as the contingency). Proofs: fork suite 54/0/4 + unit 10/10 + the live round-trip tx hashes in `FORK_PROOFS.md` §"v1.5 G2 — live spike". Must-fix (4)'s permissionless leg is scoped in `ESCAPE_HATCH_SCOPE.md` and is unblocked by this route.
